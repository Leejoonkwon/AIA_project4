...
for i in file_name_list:
    retinaface=RetinaFace.detect_faces(img_path=f"d:/project/DL/retina_data/{i}.png")
    K.append(retinaface)
...
for i in range(len(K)):
    for j in range(len(K[i])):
        ...
        img_resize.save(f"0000{i}{j}.jpg")

'''
A=[]        
for i in range(len(file_name_list)):
    retinaface=RetinaFace.extract_faces(img_path=f"d:/project/DL/retina_data/000{i}",align=True)
    A.append(retinaface)
